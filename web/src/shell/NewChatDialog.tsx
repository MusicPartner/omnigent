 ...
 * POSIX or Windows drive paths are accepted. Tilde-prefixed and relative
 * paths are rejected because the server never expands ``~`` — that's the
 * host's job, and the workspace request body must be unambiguous. Empty /
 * whitespace-only input is also rejected so submit stays disabled until the
 * user has typed something usable.
 *
...
export function normalizeWorkspacePath(path: string): string | null {
  ...
  if (/^[A-Za-z]:[\\/]/.test(trimmed)) {
    ...
    return (normalized.endsWith(":") ? `${normalized}/` : normalized).toLowerCase();
  }
